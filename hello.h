...
{
  int status;
  struct addrinfo hints,*found; //hints nous représente, *found est une liste chainé
  memset(&hints, 0, sizeof(hints)); //on initialise ces cases a 0 pour que tt ce passe bien
...
char ip[INET_ADDRSTRLEN];
  ...
  status=getaddrinfo(
    ...
    &hints,
    &found
  );//on interpréte (host, port) pour trouver des destination depuis hints qu'on stock dans found
