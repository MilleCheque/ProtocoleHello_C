...
int main(int argc, char** argv)
{
  ...
  getTCPsap(&cltsSAP,&cltsSAPlen,NULL,service);//On stock le client que l'on accept

  /* création de l'automate de connexion */
  int sock;
  if ( (sock=socket(AF_INET, SOCK_STREAM, 0))==-1 ) { //on a initialisé un flux qui communique via TCP
    fprintf(stderr,"%s: pb socket: %s\n",argv[0],strerror(errno));
    exit(1);
  }
  if ( bind(sock, &cltsSAP, cltsSAPlen)<0 ) {//on associe le socket a une ip et un port
  ...
  while (1) {//on prépare 
    ...
    socklen_t       cltSAPlen=sizeof(cltSAP);

    /* creation du flux de communication (cx) */
    if ( (cx=accept(sock, &cltSAP, &cltSAPlen))==-1 ) {//accpt est bloquante
